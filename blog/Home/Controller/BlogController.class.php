...
class BlogController extends PublicController
{
  ...
  function index($gid = "")
  {
     ...
     * 文章内容
     */
    $bloginfo = $blog->where("gid = '%d'",$gid)
      ->join('__BLOG__ ON __BLOG__.navid = __NAVI__.id')
      ->field("gid,title,date,content,naviname,navid,blog_seotitle,keywords,description")
      ->select();
    /**
     * 上一页 下一页
     */
    $blogfront = $blog->where("gid < '%d'",$gid)
      ->join('__BLOG__ ON __BLOG__.navid = __NAVI__.id')
      ->field("gid,title")
      ->find();
    $blogafter = $blog->where("gid > '%d'",$gid)
      ->join('__BLOG__ ON __BLOG__.navid = __NAVI__.id')
      ->field("gid,title")
      ->find();
    /**
     * 相关文章
     */
    $navid= $bloginfo[0]['navid'];
    $blogother = $blog ->where("navid ='%s'",$navid)->join('__BLOG__ ON __BLOG__.navid = __NAVI__.id')
      ->field("gid,title")
      ->limit("6")
      ->select();
   // showbug($blogother);
    $this->assign("front",$blogfront);
    $this->assign("after",$blogafter);
    $this->assign("other",$blogother);
  ...
  function category_index()
  {
    ...
    $str = "<tr>
              ...
              <td>
              <a class='green' href='/blog/index.php/Admin/Blog/category_add/pid/\$id.html'title='新增分类'>
              <i class='ace-icon fa fa-plus-circle bigger-100'></i>
              新增</a>&nbsp;&nbsp;
              <a class='blue' href='/blog/index.php/Admin/Blog/category_edit/id/\$id.html' title='编辑分类'>
              <i class='ace-icon fa fa-plus-circle bigger-100'></i>
              编辑</a>&nbsp;&nbsp;
